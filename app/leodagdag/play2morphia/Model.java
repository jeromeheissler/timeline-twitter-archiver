...
public class Model {
			...
			try {
				...
				while (clazz != null) {
					for (Field f : clazz.getDeclaredFields()) {
						if (f.isAnnotationPresent(Id.class)) {
							PropertyDescriptor idProperty = new BeanWrapperImpl(
									this).getPropertyDescriptor(f.getName());
							_idGetSet = Tuple(idProperty.getReadMethod(),
									idProperty.getWriteMethod());
						}
					}
				...
				if (_idGetSet == null) {
					throw new RuntimeException(
							"No @com.google.code.morphia.annotations.Id field found in class ["
									+ this.getClass() + "]");
				}
	...
	private void saveBlobs() {
		try {
			...
			for (Field blobField : blobFields) {
				...
				Blob.delete(bloblFieldName); // <= ... we delete it from
												// database
	...
	protected String computeBlobFileName(String fieldName) {
		return String.format("%s_%s_%s", getClass().getSimpleName(), _getId(),
				fieldName);
	}
}
